...
public interface VerificationDocumentRepository extends JpaRepository<VerificationDocument, UUID> {
    ...
    List<VerificationDocument> findByStatusOrderBySubmittedAtAsc(DocumentStatus status);

    Optional<VerificationDocument> findByWorkerProfileIdAndDocumentTypeAndStatus(
            UUID workerProfileId, DocumentType documentType, DocumentStatus status
    );
}
